...
public class Code04_VisibleMountains {
     ...
     * 思路： 由小山去找大山 这样就不会错过了
     * 1、从顶到低依次增大的顺序压栈
     * 2、不满足这个条件的时候 底下的可以弹出了 此时就能计算该点的数了
     * 3、不满足的全部弹出后该数压入   最后处理栈中剩余的
     * @param arr
     * @return
     */
    public static int numsOfMountains(int[] arr){
        ...
        while (index!=maxIndex){
            while (arr[index] > stack.peek().val){
                //算数据 如果times = 1 是2  如果times不是1 2*k + C N 2
                total += getNums(stack.pop().times);
            }
            //如果相等就合并
            if(arr[index] == stack.peek().val){
                stack.peek().times++;
            }else {
                //小于则加进去
        ...
        //清算第二阶段的数据
        if(stack.size() ==2){
            int nums = stack.pop().times;
            total +=  (stack.peek().times == 1 ? nums : 2*nums)+ nums*(nums-1)/2;
        }

        total += stack.peek().times * (stack.peek().times-1)/2;


        return total;
    }

    static int getNums(int times){
    ...
    public static int getNextIndex(int i, int size){
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            if (rightWay(arr) != numsOfMountains(arr)) {
                printArray(arr);
                System.out.println(rightWay(arr));
                System.out.println(numsOfMountains(arr));
                break;
            }
        }
